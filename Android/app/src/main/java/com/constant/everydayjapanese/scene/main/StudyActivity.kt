                ...
                vocabulariesForCell?.get(position)?.let { vocabularyForCell ->
                    ...
                    buttonBookmark.setOnClickListener {
                        if (vocabularyForCell.isBookmark) {
                            ...
                            bookmark.add(vocabularyForCell.vocabulary)
                        }
                        vocabularyForCell.isBookmark = !vocabularyForCell.isBookmark
                        vocabularyAdapter.notifyItemChanged(position)
                        PrefManager.getInstance().setValue(Pref.vocabularyBookmark.name, JSONManager.getInstance().encodeVocabularyJSON(bookmark))
                    }
                    buttonSound.setOnClickListener {
...
//                                vocabularyAdapter.notifyItemChanged(position)
    ...
    private var bookmark: HashSet<Vocabulary> = HashSet<Vocabulary>()
    ...
    private fun initializeVariables() {
        ...
        PrefManager.getInstance().getStringValue(
            ...
            bookmark = JSONManager.getInstance().decodeJSONtoVocabularySet(it)
        }
